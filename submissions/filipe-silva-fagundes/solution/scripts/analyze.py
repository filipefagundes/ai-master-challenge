...
def train_and_evaluate_nb(frame: pd.DataFrame, seed: int = 42) -> dict:
    """Small, transparent baseline used by the production routing layer."""
    ...
    return {
        ...
        "purpose": "roteador auditável; embeddings recuperam casos e o LLM redige",
...
def write_routing_model(frame: pd.DataFrame, train_indices: list[int], output_path: Path) -> None:
    """Compile the audited NB split into a dependency-free TypeScript artifact."""
    documents = frame["Document"].astype(str).tolist()
    document_labels = frame["Topic_group"].astype(str).tolist()
    labels = sorted(set(document_labels))
    word_counts = {label: Counter() for label in labels}
    token_totals = Counter()
    class_docs = Counter()
    vocabulary: set[str] = set()
    ...
    model = {
        "labels": labels,
        "logPriors": [math.log(class_docs[label] / total_train) for label in labels],
        "logDenominators": [math.log(token_totals[label] + vocab_size) for label in labels],
        "tokenCounts": {
            token: [word_counts[label][token] for label in labels]
            for token in sorted(vocabulary)
        },
        "metadata": {
            "method": "Multinomial Naive Bayes, Laplace alpha=1",
            "split": "80/20 stratified, seed 42",
            "trainRows": total_train,
            "vocabulary": vocab_size,
        },
    }
    ...
    output_path.write_text(
        ...
        "type RoutingModel = { labels: readonly string[]; logPriors: readonly number[]; "
        "logDenominators: readonly number[]; tokenCounts: Record<string, readonly number[]>; "
        "metadata: Record<string, string | number> };\n"
        f"const routingModel: RoutingModel = {serialized};\n\nexport default routingModel;\n",
        encoding="utf-8",
    )
...
def main() -> None:
    ...
    write_routing_model(topics, train_indices, args.output_dir / "routing-model.ts")
